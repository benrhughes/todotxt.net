...
namespace Client
{
    ...
    public class MainWindowViewModel : INotifyPropertyChanged
    {
        ...
        private bool ShouldUpdateTask (KeyEventArgs e)
        {
            ...
            if (_updating != null && e.Key == Key.Enter)
            {
                if (User.Default.RequireCtrlEnter)
                {
                    if (e.KeyboardDevice.Modifiers == ModifierKeys.Control)
        ...
        private string _prefixInfoTaskRecord
        {
            get
            {
                return "x " + DateTime.Now.ToString("HH\\:mm yyyy-MM-dd ");
            }
        }
        public void AddTaskRecord()
        {
            ...
            var item = _window.lbTasks.SelectedItems[0];
            var newTaskItem = _prefixInfoTaskRecord + item.ToString();
            var taskDetail = newTaskItem;
        ...
        private void AddTimeRecordFromTextBox ()
        {
            string taskString = _window.taskText.Text;

            var taskDetail = _prefixInfoTaskRecord + taskString;
        ...
        internal void TaskTextPreviewKeyUp(KeyEventArgs e)
        {
            ...
            if (ShouldAddRecord(e))
            {
                AddTimeRecordFromTextBox();
                _window.taskText.Text = "";
                return;
            }
